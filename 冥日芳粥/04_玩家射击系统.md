...
func _try_shoot(shoot_input: Vector2) -> void:
    ...
    var shoot_dir := shoot_input.normalized()
    var has_spawn_bullet := _fire_bullet(shoot_dir)
    if has_spawn_bullet:
...
func _try_auto_spiral_shoot() -> void:
    ...
    var spiral_dir := Vector2.RIGHT.rotated(spiral_phase)
    var has_spawn_bullet := _fire_bullet(spiral_dir)
    if has_spawn_bullet:
        shoot_timer.start(_get_effective_fire_interval())
```
...
```gdscript
func _fire_bullet(shoot_dir: Vector2) -> bool:
    if current_shot_pattern == SHOT_PATTERN_SPIRAL:
        var has_spawn_up := _spawn_bullet(shoot_dir)
        var has_spawn_down := _spawn_bullet(shoot_dir.rotated(PI))
        spiral_phase = wrapf(spiral_phase + SPIRAL_PHASE_STEP, 0.0, TAU)
        return has_spawn_up or has_spawn_down
    return _spawn_bullet(shoot_dir)
```
...
```gdscript
func _spawn_bullet(shoot_dir: Vector2) -> bool:
    ...
    bullet.global_position = global_position + shoot_dir * bullet_spawn_distance
    return true
```
...
func _get_effective_fire_interval() -> float:
    return _get_effective_fire_rate_multiplier() * fire_interval
```
